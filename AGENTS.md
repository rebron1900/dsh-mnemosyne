...
- **描述**：DeepSeek Harness（DSH）的 Mnemosyne 记忆插件 — Profile Bundle 形态，注册 5 个记忆工具与 1 个内嵌技能，提供 Settings 独立面板（CLI 状态/统计/一键安装/测试/配置/恢复默认），数据落 `~/.dsh/mnemosyne`，CLI 通过 `uv tool install` 自动安装。移植自 `mnemosyne-oss/Pi-mnemosyne`。
