...
async function handleInitError(error){
  ...
  let status = null;
  try {
    const r = await fetch('/mnemosyne/dashboard/api/auth/status', { cache: 'no-store' });
    status = await r.json();
...
async function loadStats(){
  const s = await api('/mnemosyne/dashboard/api/stats');
...
async function refreshCurrentView(){
  const button = $('#mobileRefreshButton');
  if(button?.disabled) return;
  if(button){ button.disabled = true; button.classList.add('is-refreshing'); }
  try {
    const tab = currentRoute?.tab || 'overview';
    await loadStats();
    if(tab !== 'overview') switchTab(tab, {push:false});
  } finally {
...
async function loadLiveMemoryStream(append=false){
  ...
  try {
    const data = await api(`/mnemosyne/dashboard/api/memories?${params.toString()}`);
...
async function loadRuntimeDiagnostics(){
  try {
    renderRuntimeDiagnostics(await api('/mnemosyne/dashboard/api/runtime/status'));
  } catch(e) {
    const el = $('#runtimeDiagnostics');
...
async function loadRealtimePanel(){
  try {
    realtimeState.status = await api('/mnemosyne/dashboard/api/realtime/status');
...
async function initRealtime(){
  // Live sync diagnostics live on the upstream Settings panel, which is not
  // part of the dsh-mnemosyne read-only integration. Keep the realtime stream
  // disabled so the dashboard boots without an extra SSE connection.
  return;
}
...
async function openMemoryDetail(memoryId, opts={}){
  ...
  const item = (await api('/mnemosyne/dashboard/api/memory?id=' + encodeURIComponent(memoryId))).item;
...
async function openSessionDetail(sessionId, opts={}){
  ...
  const data = await api(`/mnemosyne/dashboard/api/session?id=${encodeURIComponent(sessionId)}&limit=200`);
  const c = data.counts || {};
  showHtmlDetail(`
...
async function loadTriples(){
  ...
  const data = await api(`/mnemosyne/dashboard/api/triples?q=${q}&limit=300`);
...
async function loadConsolidations(){
  const data = await api('/mnemosyne/dashboard/api/consolidations?limit=200');
...
async function loadGlobalSearch(){
  ...
  try{
    const data = await api(`/mnemosyne/dashboard/api/search?q=${encodeURIComponent(q)}&limit=30`);
...
async function loadRecallDebug(){
  ...
  const data = await api(`/mnemosyne/dashboard/api/recall-debug?q=${encodeURIComponent(q)}&limit=30`);
...
async function loadTimeline(){
  ...
  const data = await api(`/mnemosyne/dashboard/api/timeline?q=${encodeURIComponent(q)}&group=${encodeURIComponent(group)}&limit=300`);
...
async function loadTodayDigest(day=''){
  ...
  const data = await api(`/mnemosyne/dashboard/api/digest/today?limit=80${suffix}`);
  const c = data.counts || {};
...
async function loadPatternInsights(){
  const data = await api('/mnemosyne/dashboard/api/patterns?limit=10');
...
async function loadProfile(){
  const [data] = await Promise.all([api('/mnemosyne/dashboard/api/profile/inferred?limit=10'), loadPatternInsights()]);
...
async function loadReviewPage(append=false){
  const data = await api(`/mnemosyne/dashboard/api/review?${reviewFilterParams().toString()}`);
...
async function loadLifecycle(){
  const data = await api('/mnemosyne/dashboard/api/lifecycle?limit=80');
  const queues = data.queues || {};
...
async function loadConstellation(){ drawConstellation(await api('/mnemosyne/dashboard/api/constellation?limit=240')); }
async function loadDiagnostics(){
  const diag = await api('/mnemosyne/dashboard/api/diagnostics');
...
async function refreshAuthState(){
  authState = { config: { host: '127.0.0.1', memory_admin_enabled: false }, auth_enabled: false, authenticated: true };
  return authState;
}
...
async function loadGraph(){
  ...
  drawGraph(await api(`/mnemosyne/dashboard/api/graph?q=${q}&limit=300`));
}
...
function loadThreeModule(){
  if(!threeModulePromise) threeModulePromise = import('/mnemosyne/dashboard/static/vendor/three.module.min.js');
...
async function loadThreeVisualiser(){ renderThreeVisualiser(await api('/mnemosyne/dashboard/api/constellation?limit=320')); }
...
async function loadMemoryPalace(){ renderMemoryPalace(await api('/mnemosyne/dashboard/api/constellation?limit=360')); }
...
async function loadMemoria(){
  ...
  const stats = await api('/mnemosyne/dashboard/api/memoria/stats');
  ...
  loadMemoriaTable('memoriaFacts', '/mnemosyne/dashboard/api/memoria/facts', 'memoriaFactsList', 'memoriaFactsCount');
  loadMemoriaTable('memoriaTimelines', '/mnemosyne/dashboard/api/memoria/timelines', 'memoriaTimelinesList', 'memoriaTimelinesCount');
  loadMemoriaTable('memoriaInstructions', '/mnemosyne/dashboard/api/memoria/instructions', 'memoriaInstructionsList', 'memoriaInstructionsCount');
  loadMemoriaKg();
  loadMemoriaTable('memoriaPreferences', '/mnemosyne/dashboard/api/memoria/preferences', 'memoriaPreferencesList', 'memoriaPreferencesCount');
}
...
async function loadMemoriaKg(){
  ...
  const r = await api(`/mnemosyne/dashboard/api/memoria/kg?q=${encodeURIComponent(q)}&limit=200`);
  const items = r.items || [];
...
$('#memoriaFactsSearch').onclick = () => loadMemoriaTable('memoriaFacts', '/mnemosyne/dashboard/api/memoria/facts', 'memoriaFactsList', 'memoriaFactsCount');
$('#memoriaFactsQuery').onkeydown = e => { if(e.key==='Enter') $('#memoriaFactsSearch').click(); };
$('#memoriaTimelinesSearch').onclick = () => loadMemoriaTable('memoriaTimelines', '/mnemosyne/dashboard/api/memoria/timelines', 'memoriaTimelinesList', 'memoriaTimelinesCount');
$('#memoriaTimelinesQuery').onkeydown = e => { if(e.key==='Enter') $('#memoriaTimelinesSearch').click(); };
$('#memoriaInstructionsSearch').onclick = () => loadMemoriaTable('memoriaInstructions', '/mnemosyne/dashboard/api/memoria/instructions', 'memoriaInstructionsList', 'memoriaInstructionsCount');
...
$('#memoriaPreferencesSearch').onclick = () => loadMemoriaTable('memoriaPreferences', '/mnemosyne/dashboard/api/memoria/preferences', 'memoriaPreferencesList', 'memoriaPreferencesCount');
$('#memoriaPreferencesQuery').onkeydown = e => { if(e.key==='Enter') $('#memoriaPreferencesSearch').click(); };
$('#closeDetail').onclick = () => closeDetail();
$('#loginButton').onclick = async () => {
  try { await postJson('/mnemosyne/dashboard/api/auth/login', {password: $('#loginPassword').value}); hideLogin(); $('#loginError').textContent=''; await refreshAuthState(); loadStats(); }
...
$('#saveRuntimeConfig').onclick = async () => {
  try {
    ...
    const r = await postJson('/mnemosyne/dashboard/api/config', body);
    const cfg = r.config || {};
    $('#configHost').value = cfg.host || '';
    $('#configPort').value = cfg.port || '';
    $('#configDbPath').value = cfg.db_path || '';
...
$('#saveAuth').onclick = async () => {
  try { const body = {auth_enabled: $('#authEnabled').checked}; if($('#authPassword').value) body.password = $('#authPassword').value; const r = await postJson('/mnemosyne/dashboard/api/config', body); $('#authPassword').value=''; $('#authStatus').textContent = r.message || 'Saved'; }
  catch(e){ $('#authStatus').textContent = e.message; }
};
$('#clearAuth').onclick = async () => {
  try {
    const r = await postJson('/mnemosyne/dashboard/api/config', {clear_password:true});
...
$('#saveMemoryAdmin').onclick = async () => {
  try { const r = await postJson('/mnemosyne/dashboard/api/config', {memory_admin_enabled: $('#memoryAdminEnabled').checked}); authState.config = r.config || {}; $('#memoryAdminStatus').textContent = r.message || 'Saved'; await loadAuthStatus(); }
  catch(e){ $('#memoryAdminStatus').textContent = e.message; }
};
$('#createBackup').onclick = async () => {
  try { const r = await postJson('/mnemosyne/dashboard/api/admin/backup', {}); $('#memoryAdminStatus').textContent = `Backup created: ${r.backup.path}`; }
  catch(e){ $('#memoryAdminStatus').textContent = e.message; }
};
$('#viewAuditLog').onclick = async () => {
  try { const r = await api('/mnemosyne/dashboard/api/admin/audit?limit=50'); showDetail(r.items, 'Memory audit log'); }
  catch(e){ $('#memoryAdminStatus').textContent = e.message; }
};
$('#retryBootstrap').onclick = () => bootstrapDashboard().catch(handleInitError);
$('#copyBootError').onclick = copyBootErrorDetails;
$('#logoutAuth').onclick = async () => { await postJson('/mnemosyne/dashboard/api/auth/logout', {}); showLogin(); };
