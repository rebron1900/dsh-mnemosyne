...
window.__ModuleLoader__.load({ id: "dsh-mnemosyne", factory: (require) => {
  ...
  const LOCALE = {
    zh: {
      ...
      groupAuto: "自动记忆",
      groupAuto_hint: "自动记忆默认跟随 Mnemosyne Hermes 集成启用。可分别关闭自动存储、预取或 prompt 声明；session-scoped 记忆通过显式 global 记忆跨会话共享。",
      f_promptSection: "Prompt 声明段",
      f_promptSection_hint: "在 system prompt 注入 '# Mnemosyne Memory' 段，告诉模型记忆工具可用。",
      f_autoSync: "自动存储对话",
      f_autoSync_hint: "每轮对话后自动将真实用户消息存入 Mnemosyne；默认仅保存用户消息，需在 sync_roles 中加入 assistant 才保存助手回复。",
      f_syncTurnUserLimit: "User 同步长度上限",
      f_syncTurnUserLimit_hint: "自动同步保存的 user 最大字符数，默认 500，与 Hermes 一致；设为 0 表示不截断。该设置保留完整原文，不会自动分块或摘要。",
      f_syncTurnAssistantLimit: "Assistant 同步长度上限",
      ...
      f_sleepThreshold_hint: "config.yaml 键 sleep_threshold，默认 50。每 10 个 durable turn 检查一次，工作记忆达到该条目数后触发自动整合。",
    ...
    en: {
      ...
      groupAuto_hint: "Automatic memory follows the Mnemosyne Hermes integration by default. Disable prompt, sync, or prefetch independently; explicit global memories remain the cross-session durable surface.",
      f_promptSection: "Prompt section",
      f_promptSection_hint: "Inject a '# Mnemosyne Memory' section into the system prompt telling the model that memory tools are available.",
      f_autoSync: "Auto-sync conversation",
      f_autoSync_hint: "Automatically store real user messages to Mnemosyne after each turn; user-only is the default, and assistant must be added to sync_roles to store assistant replies.",
      f_syncTurnUserLimit: "User sync length limit",
      f_syncTurnUserLimit_hint: "Maximum user characters stored by auto-sync. Defaults to 500 for Hermes parity; set 0 to disable truncation. This preserves the full original text without chunking or summarizing it.",
      f_syncTurnAssistantLimit: "Assistant sync length limit",
      ...
      f_sleepThreshold_hint: "config.yaml key sleep_threshold, default 50. The provider checks every 10 durable turns and consolidates when working-memory entries reach this threshold.",
  ...
  //       omitted for DSH-only plugin and automatic-memory fields.
  const FIELDS = [
